...
Finally, there are some other values in the file. The first 4 bytes are always set to 0, as they are written to when loaded into memory. There is a field that likely corresponds to the index of the section where looping starts, and there is also a field (either 0 or -1) that has no clear effect currently.
